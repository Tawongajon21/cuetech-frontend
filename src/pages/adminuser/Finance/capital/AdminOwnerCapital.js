...
const Adminownercapital = () => {
    ...
    const getCapital = () => {
        axios.get(`finance/capital/all/${id}/`, config())
            .then(res => {
                res.data.response === 1 ? setcapital(res.data.result) : alert(res.data.message + ". If This messages persists contact admin")
            })
            .catch(err => {
                console.log(err)
            })
    }
    const deleteCapital = (cid) => {
        const ask = window.confirm("Are you sure you want to delete this record ?")
        if (ask === true) {
            axios.delete(`finance/capital/delete/${cid}/${id}/`, config())
                .then(res => {
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td className='text-center'>
                                                                <div className='button-group'>
                                                                    <Link to={{ pathname: `/adminuser/farmers/capital/update/${cap.id}/${id}` }} className="btn btn-sm btn-info mr-2"> <i className="fa fa-edit"></i></Link>
                                                                    <button onClick={(e) => (deleteCapital(cap.id))} className="btn btn-sm btn-danger"> <i className="fa fa-trash"></i></button>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                    ))}
...
export default Adminownercapital;
