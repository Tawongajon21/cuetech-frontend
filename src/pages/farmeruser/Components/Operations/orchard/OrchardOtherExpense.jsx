...
function OrchardOtherExpense({dat}) {
    const [orchardsotherexpense, setorchardsotherexpense] = useState([]);
    useEffect(() => {
        getOrchardsotherexpense()
    }, [])

    const getOrchardsotherexpense = () => {
        axios.get(`operations/orchards/otherexpenses/all/${dat.id}/`, config())
            .then(res => {
                console.log(res.data)
                res.data.response === 1 ? setorchardsotherexpense(res.data.result) : alert(res.data.message + ". If This messages persists contact admin")
            })
            .catch(err => {
                console.log(err)
            })
    ...
    const deleteOrchardsinfo = (pk) => {
        const url = `operations/orchards/otherexpenses/delete/${pk}/`
    ...
    <div className='col-md-12 mb-2'>
            <div className="card direct-chat direct-chat-primary">
                <div className="card-header ui-sortable-handle bg-primary">
                    <h3 className="card-title">Orchards Other Expenses</h3>

                    <div className="card-tools">
                        <Link to={{ pathname: `/adminuser/farmers/orchard/otherexpenses/add/${dat.id}/${dat.uid}` }}
                                ...
                                <tr>
                                    ...
                                    <td className='text-center'>
                                        <div className='button-group'>
                                            <Link to={{ pathname: `/adminuser/farmers/orchard/otherexpenses/update/${tan.id}/${dat.uid}` }}
...
export default OrchardOtherExpense